...
import { Wifi, Plus, X, Shield, Globe, Play, Square, Trash2, Pencil, Film } from "lucide-react";
...
export default function StreamsPage() {
  ...
  const defaultForm = { title: "", platform: "youtube", streamKey: "", rtmpUrl: "", quality: "720p", mode: "loop", videos: [] as string[] };
  ...
  const openEdit = (stream: StreamItem) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        if (!form.streamKey) delete body.streamKey;
        await api(`/streams/${editingId}`, { method: "PUT", body });
      } else {
  ...
  const toggleVideo = (videoId: string) => {
    setForm((prev) => ({
      ...
      videos: prev.videos.includes(videoId)
        ? prev.videos.filter((v) => v !== videoId)
        : [...prev.videos, videoId],
    }));
  };
          ...
          {streams.map((stream) => (
            <div key={stream._id} className="card p-5 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 text-xs text-gray-500">
                    ...
                    {stream.videos && stream.videos.length > 0 && (
                      <span className="flex items-center gap-1 text-purple-400">
                        <Film className="w-3 h-3" /> {stream.videos.length} video{stream.videos.length > 1 ? "s" : ""}
                      </span>
                    )}
              ...
              <div className="flex items-center gap-2">
                {stream.status !== "live" && (
                  <button onClick={() => openEdit(stream)} className="p-2 rounded-lg bg-purple-500/10 text-purple-400 hover:bg-purple-500/20 transition" title="Edit">
                    <Pencil className="w-4 h-4" />
                  </button>
                )}
                {stream.status === "live" ? (
                  <button onClick={() => handleStop(stream._id)} className="p-2 rounded-lg bg-red-500/10 text-red-400 hover:bg-red-500/20 transition" title="Stop">
                    <Square className="w-4 h-4" />
                  </button>
                ) : (
                  <button onClick={() => handleStart(stream._id)} className="p-2 rounded-lg bg-green-500/10 text-green-400 hover:bg-green-500/20 transition" title="Start">
                    <Play className="w-4 h-4" />
                  </button>
                )}
                <button onClick={() => handleDelete(stream._id)} className="p-2 rounded-lg text-gray-600 hover:text-red-400 hover:bg-red-500/10 transition" title="Delete">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-[#1a1333] border border-purple-900/50 rounded-2xl p-7 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold">{editingId ? "Edit Stream" : "Create Stream"}</h2>
              <button onClick={() => { setShowModal(false); setEditingId(null); }} className="text-gray-500 hover:text-white p-1">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                <label className="text-sm font-medium text-gray-300 mb-1.5 block">
                  Stream Key {editingId && <span className="text-gray-600 text-xs">(leave empty to keep current)</span>}
                </label>
                <input
                  type="password"
                  value={form.streamKey}
                  onChange={(e) => setForm({ ...form, streamKey: e.target.value })}
                  placeholder={editingId ? "Enter new key or leave empty" : "Paste your stream key from YouTube/Facebook"}
                  required={!editingId}
                  className="w-full bg-[#0f0a1e] border border-purple-900/50 rounded-xl py-3 px-4 text-white placeholder-gray-600 focus:border-purple-500 focus:outline-none transition"
                />
              ...
              <div>
                <label className="text-sm font-medium text-gray-300 mb-1.5 block flex items-center gap-2">
                  <Film className="w-4 h-4 text-purple-400" /> Select Videos for Live Stream
                </label>
                {videos.length === 0 ? (
                  <p className="text-gray-500 text-sm bg-[#0f0a1e] border border-purple-900/30 rounded-xl py-4 px-4 text-center">
                    No videos uploaded yet. Upload videos first from Video Gallery.
                  </p>
                ) : (
                  <div className="bg-[#0f0a1e] border border-purple-900/30 rounded-xl p-3 max-h-40 overflow-y-auto space-y-2">
                    {videos.map((video) => (
                      <label
                        key={video._id}
                        className={`flex items-center gap-3 p-2.5 rounded-lg cursor-pointer transition ${
                          form.videos.includes(video._id)
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={form.videos.includes(video._id)}
                          onChange={() => toggleVideo(video._id)}
                          className="rounded border-purple-900 text-purple-500 focus:ring-purple-500"
                        />
                        <Film className="w-4 h-4 text-purple-400 flex-shrink-0" />
                        <span className="text-sm text-gray-300 truncate">{video.title || video.originalName}</span>
                      </label>
                    ))}
                  </div>
                )}
                {form.videos.length > 0 && (
                  <p className="text-purple-400 text-xs mt-1.5">{form.videos.length} video{form.videos.length > 1 ? "s" : ""} selected</p>
                )}
              </div>
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                >
                  {loading ? (editingId ? "Saving..." : "Creating...") : (editingId ? "Save Changes" : "Create Stream")}
                </button>
              </div>
            </form>
          </div>
